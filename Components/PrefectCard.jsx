...
function PrefectCard({posts,candidate,isClicked,setIsClicked,id,setNortification}){
    ...
    return(
        <>
        <div onClick={() => handleClick(candidate.Candidate_Name,candidate.prefectorial_Post)} key={candidate.id} className={`flex mt-20 flex-col w-[98%] sm:w-[300px] px-5 text-white items-center hover:cursor-pointer justify-evenly shadow-2xl bg-[#101540] rounded-md ${isClicked ? 'border-2 border-[#5478FF]':''}`}>
